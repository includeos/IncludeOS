...
To achieve this behavior you have to add option(libsyslogd "" ON) to your CMakeLists.txt file.

The default behavior for IncludeOS's syslog implementation is however to send the data to printf, which is
what you get if you don't add option(libsyslogd "" ON) to your CMakeLists.txt file (see the syslog_default integration test).
