...
class MuslConan(ConanFile):
    ...
    def build(self):
        ...
        env_build.configure(configure_dir=self.source_folder+"/musl",target=str(self.settings.arch)+"-elf",args=["--enable-debug","--disable-shared"]) #what goes in here preferably
        env_build.make()
        env_build.install()
