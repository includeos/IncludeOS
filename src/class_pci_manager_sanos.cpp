...
resource* add_resource(unit* unit, unsigned short type, unsigned short flags, unsigned long start, unsigned long len){
  ...
  printf("Adding resource, classcode: 0x%lx - start: 0x%lx, len: 0x%lx \n",unit->classcode,start,len)
;
  
  if (unit->unitcode == 0x1af41000 && type == RESOURCE_IO) {
    printf("VIRTIO PCI IO-resource \n");
    printf("Host features (from iobase 0x%lx): 0x%lx \n",start,inpd(start + VIRTIO_PCI_HOST_FEATURES));
  }
...
void PCI_manager::init(){
  ...
  for(int i=0;i<unitcount;i++){
    ...
    if (unitlist[i].unitcode == 0x1af41000) {
      virtio_index=i;
    }
  }
  ...
  if (virtio_index >= 0)
    printf("Virtio IOBase: 0x%lx \n",unitlist[virtio_index]);
