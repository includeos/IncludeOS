...
void Arp::bottom(Packet_ptr pckt) {
  ...
  case H_request: {
    debug2("\t ARP REQUEST: ");
    debug2("%s is looking for %s\n", hdr->sipaddr.c_str(),hdr->dipaddr.c_str());
  ...
  case H_reply: {
    debug2("\t ARP REPLY: %s belongs to %s\n", hdr->sipaddr.c_str(), hdr->shwaddr.c_str());
...
void Arp::cache(IP4::addr& ip, Ethernet::addr& mac) {
  ...
  if (entry != cache_.end()) {
    debug2("Cached entry found: %s recorded @ %llu. Updating timestamp\n",
          entry->second.mac_.c_str(), entry->second.timestamp_);
...
int Arp::arp_resolve(Packet_ptr pckt) {
  ...
  auto req = view_packet_as<PacketArp>(inet_.createPacket(sizeof(header)));
  req->init(mac_, ip_);
