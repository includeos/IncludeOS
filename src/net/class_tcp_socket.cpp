...
void TCP::Socket::syn_ack(std::shared_ptr<Packet>& pckt){
  ...
  hdr->ack_nr = htons(hdr->seq_nr + 1); // Required
  hdr->seq_nr = htons(42); // Random
  
  // Set source port
  hdr->sport = htons(local_port_);  
  // Try to let the underlying stack set the source IP
  //full_header->ip_hdr.saddr = local_stack_ 
