...
void Inet4::error_report(const ICMP_error& err, Packet_ptr orig_pckt) {
  ...
  if (pckt_ip4->ip_protocol() == Protocol::UDP) {
    ...
    udp_.error_report(err,
      Quadruple{pckt_udp->ip_src(), pckt_udp->src_port(), pckt_udp->ip_dst(), pckt_udp->dst_port()});
  } else if (pckt_ip4->ip_protocol() == Protocol::TCP) {
    auto pckt_tcp = static_unique_ptr_cast<tcp::Packet>(std::move(pckt_ip4));
    tcp_.error_report(err,
      Quadruple{pckt_tcp->ip_src(), pckt_tcp->src_port(), pckt_tcp->ip_dst(), pckt_tcp->dst_port()});
  }
}
